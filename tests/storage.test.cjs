...
test("Upload mp3 file", async function (t) {
  ...
  test("Should start formation of mp3 file", async function (t) {
    const file_path = "test/files/Rick Astley - Never Gonna Give You Up.mp3";
    const asset_filename = path.basename(file_path);
    const asset_content_type = mime.getType(file_path);
